...
type parser struct {
	...
	scanLook func(pos token.Pos, tok token.Token, lit string) bool
}

func (p *parser) init(fset *token.FileSet, filename string, src []byte, mode Mode, opts []interface{}) {
	...
	var looker scanLooker
	for _, opt := range opts {
		if x, ok := opt.(func(token.Pos, token.Token, string) bool); ok {
			looker = append(looker, x)
		}
	}

	if len(looker) == 1 {
		p.scanLook = looker[0]
	} else if len(looker) > 1 {
		p.scanLook = looker.invoke
	}

	p.next()
}

type scanLooker []func(token.Pos, token.Token, string) bool

func (looker scanLooker) invoke(pos token.Pos, tok token.Token, lit string) bool {
	ends := 0
	for i, x := range looker {
		if x == nil {
			ends++
			continue
		}
...
func (p *parser) next0() {
	...
	if p.scanLook != nil {
		if !p.scanLook(p.pos, p.tok, p.lit) {
			p.scanLook = nil
		}
	}
}
